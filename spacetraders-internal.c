...
void st_error(char* s) { printf("%s: %s\n", s, ST_ERROR); }
...
struct curl_slist* add_headers(st_client_t* client, char** headers,
                               int length) {
...
char* api_header_get(st_client_t* client, char* route, char** headers,
                     int length) {
  char* url = make_url(route);
  curl_easy_setopt(client->api->inner, CURLOPT_URL, url);
  curl_easy_setopt(client->api->inner, CURLOPT_HTTPGET, 1L);
